...
export default function AdminPage() {
  ...
  async function addGuest() {
    ...
    if (isNaN(size) || size < 1 || size > 50) {
      setFormError("Party size must be 1–50.");
      return;
    }
  ...
  async function handleCsvImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const rows = lines
      .map((line) => {
        const [name, party_size, email, notes] = line
          .split(",")
  ...
  const checkedInIds = new Set(
    scans.filter((s) => s.status === "valid").map((s) => s.guest_id),
  );
        ...
        <div className={styles.sidebarTop}>
          <div className={styles.brand}>
            ...
            >
              ...
              <path
                d="M14 14h.01M18 14h.01M14 18h.01M18 18h.01"
                strokeWidth="2"
                strokeLinecap="round"
              />
            </svg>
        ...
        {tab === "guests" && (
          <>
            ...
            <p className={styles.csvHint}>
              CSV format: <code>name, party_size, email, notes</code> (header
              row required)
            </p>
                    ...
                    {filteredGuests.map((guest) => {
                      ...
                      return (
                        <tr key={guest.id}>
                          ...
                          <td>
                            {guest.party_size === 1 ? "1" : guest.party_size}
                          </td>
                          <td>
                            <span
                              className={`${styles.statusPill} ${checked ? styles.checked : styles.notChecked}`}
                            >
